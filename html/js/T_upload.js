	...
	$.Tupload = {
		...
		init:function(defaults){
			...
			$(".uploading-img li").mouseenter(function(){
				$(this).find(".uploading-tip").stop().animate({height:'20px'},200);
			});
		...
		createHtml:function(defaults){
			...
			html += 	'<p>'+title == '' ? '注：700*700 以上的图片可以在宝贝详情页主图提供图片放大功能': title+'</p>';
			html += 	'<img id="img_srczz" class="new-upload" src="../images/imgadd.png"/>';
		...
		imgLoad:function(i,file){
			...
			$(r).load(function(){
				...
				var temping = $("#img_src"+0).attr('src');
				$("#img_srczz").attr("src",temping);
